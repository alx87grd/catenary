...
def list_to_np_array(lst):
    ...
    for i in range(len(lst)):
        ...
        lst_x = np.array([np.pad(lst[i][0], (0, length - len(lst[i][0])), constant_values=np.nan) ])
        lst_y = np.array([np.pad(lst[i][1], (0, length - len(lst[i][0])), constant_values=np.nan) ])
        lst_z = np.array([np.pad(lst[i][2], (0, length - len(lst[i][0])), constant_values=np.nan) ])
...
if __name__ == "__main__":
    ...
    for bagname in os.listdir('rosbag'):
        if bagname.endswith('.bag'):
            bagname = bagname[:-4]
            if not os.path.exists('rosbag/' + bagname):
                print(bagname)

                line_pts, drone_pos, timestamps = rosbag_to_array(bagname, '/filtered_cloud_points')
                velodyne_pts, _, _ = rosbag_to_array(bagname, '/velodyne_points')
                ...
                os.mkdir(('rosbag/' + bagname))

                np.save(('rosbag/' + bagname + '/filtered_cloud_points'), line_pts)
                np.save(('rosbag/' + bagname + '/drone_pose'), drone_pos)
                np.save(('rosbag/' + bagname + '/velodyne_points'), velodyne_pts)
                np.save(('rosbag/' + bagname + '/timestamps'), timestamps)
